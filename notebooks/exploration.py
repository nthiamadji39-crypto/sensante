"""
SénSanté - Exploration du dataset patients_dakar.csv
...
# Correction de " diagnostic " en "diagnostic"
diag_counts = df["diagnostic"].value_counts()
for diag, count in diag_counts.items():
    pct = (count / len(df)) * 100
    print(f"{diag:15s} : {count:3d} patients ({pct:.1f}%)")
